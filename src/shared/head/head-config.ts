...
const speculativeFootprint = {
  ...
  // its own attention cache, rendered by serve (-ctkd/-ctvd): the formats and the K (and V) elements one pooled token
  // adds, the bytes the tier check charges following from them as the target's do (cache-formats.ts)
  cache: v.strictObject({
    k: v.picklist(KV_TYPES),
    v: v.picklist(KV_TYPES),
    kv_elements_per_token: posInt,
  }),
...
export const TierSchema = v.strictObject({
  ...
  // the cache formats this tier serves in place of the head's [cache] ones, each on its own (cache-formats.ts)
  cache: v.optional(
    v.strictObject({
...
export const HeadSchema = v.strictObject({
  ...
  // the model's two caches: the formats every tier serves unless it names its own, and the element counts their
  // bytes follow from (cache-formats.ts); serve renders --cache-type-k/-v and -cts from these, never runtime.args
  cache: v.strictObject({
    k: v.picklist(KV_TYPES),
    v: v.picklist(KV_TYPES),
    s: v.picklist(STATE_TYPES),
    ...
    state_elements_per_copy: v.pipe(int, v.minValue(0)), // one sequence's recurrent state, the part -cts narrows
    state_fixed_mib: v.pipe(v.number(), v.minValue(0)), // the part of it that stays f32 whatever -cts says
  }),
  ...
  runtime: v.strictObject({
    args: v.array(v.string()), // how the pack is loaded: template, checkpoint grid, the engine's other switches ([cache] holds the formats)
...
export function headInvariants(head: HeadConfig): string[] {
  ...
  if (head.cache.mean_center && !tiers.some((tier) => tierCache(head, tier).k === "q4_0")) {
    violations.push(
      "cache.mean_center is declared but no tier serves a q4_0 K, the only one it is passed with: it would never reach the engine",
    );
  }
...
export function tierNeedMiB(head: HeadConfig, tier: Tier): number {
  ...
  const cache = tierCache(head, tier);
  const state = stateMiBPerCopy(head, cache); // one sequence's recurrent state, in the tier's format
  ...
  const draftMiB = draft
    ? draft.weights_mib +
      draft.overhead_mib +
      perToken(draftKvBytesPerToken(draft.cache) + draft.bytes_per_token) +
      tier.slots * geometry.compute_per_output_row_mib * draft.n_max +
      (rollsBackRecurrentState.has(draft.type) ? tier.slots * state * draft.n_max : 0)
    : 0;
  return (
    perToken(kvBytesPerToken(head, cache) + geometry.compute_bytes_per_token) +
