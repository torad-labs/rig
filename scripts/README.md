...
tsconfig's `scripts` include. `e2e-driver-only.sh` is the gate a prebuilt engine passes before it is
published: this checkout packed and installed in a fresh ubuntu:24.04 container that has one card
and only the driver, then `rig prepare`, `rig build` and a decode (it needs docker with the NVIDIA
CDI devices).
